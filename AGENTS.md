...
The skill is split into 3 sub-skills to keep LLM context lean. The CLI (`scripts/code2database_builder.py`, 247 subcommands + 8 scanner subcommands) is shared — all commands are accessible regardless of sub-skill activation.

| Sub-skill | Trigger | Purpose |
|-----------|---------|---------|
| `Code2Database` (core) | `/Code2Database` | Build + browse — always loaded. 26 Tier-1 high-weight commands. |
