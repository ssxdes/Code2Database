...
The skill is split into **3 sub-skills** to keep LLM context lean. Each sub-skill has its own `SKILL.md` exposing only the commands relevant to its layer. The CLI (`scripts/code2database_builder.py`) is shared — all 258 commands (250 builder + 8 scanner) are accessible regardless of which sub-skill is active.
...
| `scripts/code2database_builder.py` | Builder CLI entry point (main command hub, 250 builder + 8 scanner = 258 commands organized into 3 sub-skills: `/Code2Database` core, `/Code2Database-analysis`, `/Code2Database-ops`) |
