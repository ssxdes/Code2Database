...
Distributed as a Python-based skill with 3 sub-skills: `/Code2Database` (core, always loaded — 27 Tier-1 commands, `c2d` umbrella first), `/Code2Database-analysis` (deep semantic analysis, on-demand), `/Code2Database-ops` (graph editing + ops, on-demand). 260 CLI commands total (252 builder + 8 scanner), 83 MCP tools (55 base + 28 design-report) (36 `code2database_*` + 19 `cgdb_*`). CLI entry points: `scripts/code2database_builder.py`, `scripts/code2database_scanner.py`. Daemon entry point: `daemon-start`. Goal-oriented umbrella: `c2d setup/session/ask/capture/freshen/report` wraps the lifecycle behind one command (`c2d recipes` lists the ask routing recipes).
...
The skill is split into **3 sub-skills** to keep LLM context lean. Each sub-skill has its own `SKILL.md` exposing only the commands relevant to its layer. The CLI (`scripts/code2database_builder.py`) is shared — all 260 commands (252 builder + 8 scanner) are accessible regardless of which sub-skill is active.
...
| `scripts/code2database_builder.py` | Builder CLI entry point (main command hub, 252 builder + 8 scanner = 260 commands organized into 3 sub-skills: `/Code2Database` core, `/Code2Database-analysis`, `/Code2Database-ops`) |
