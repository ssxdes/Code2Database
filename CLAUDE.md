...
The skill is split into **3 sub-skills** to keep LLM context lean. Each sub-skill has its own `SKILL.md` exposing only the commands relevant to its layer. The CLI (`scripts/code2database_builder.py`) is shared — all 251 commands (243 builder + 8 scanner) are accessible regardless of which sub-skill is active.
...
| `scripts/code2database_builder.py` | Builder CLI entry point (main command hub, 243 builder + 8 scanner = 251 commands organized into 3 sub-skills: `/Code2Database` core, `/Code2Database-analysis`, `/Code2Database-ops`) |
