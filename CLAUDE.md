...
| `scripts/code2database_builder.py` | Builder CLI entry point (main command hub, 122 commands organized into 3 sub-skills: `/Code2Database` core, `/Code2Database-analysis`, `/Code2Database-ops`) |
...
Exposes 48 MCP tools (30 `code2database_*` + 18 `cgdb_*`) over stdio transport for real-time LLM agent queries. The 18 `cgdb_*` tools query the cgdb (code graph database) layer directly when the clang extraction backend is enabled. Daemon mode provides additional freshness tools via Unix socket at `/tmp/code2database-daemon-<project>.sock`:
