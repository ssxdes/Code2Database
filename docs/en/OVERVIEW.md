...
All 258 CLI commands are accessible via the shared `scripts/code2database_builder.py` regardless of which sub-skill is active. The split is purely about LLM context economy: a 4K-token core skill is always useful; a 20K-token analysis skill should only be loaded when the user asks about races or invariants.
...
│  CLI: scripts/code2database_builder.py (250 CLI commands, 258 total with 8 scanner) │
└──────────────────────────────┬───────────────────────────────────────┘
                               │
                               ▼  (optional)
┌──────────────────────────────────────────────────────────────────────┐
...
```
scripts/
├── code2database_builder.py      ← CLI entry point (250 CLI commands, argparse routing)
...
### Query & Analysis
- 258 CLI commands (3 sub-skills: core 25, analysis 13, ops 23 Tier-1)
