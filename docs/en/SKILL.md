...
- **Direct cgdb table queries** (clang backend) → 18 `cgdb_*` MCP tools (search_symbols, find_invokers, find_invoked, get_definition, get_function_body, get_struct_layout, find_type_definition, find_ops_impls, find_cfg_paths, find_data_flow, find_aliases, find_lock_held_calls, check_race_condition, find_configs_for, find_nodes_under_config, index_status, time_travel_query, list_versions)
