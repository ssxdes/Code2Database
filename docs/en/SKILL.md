---
name: Code2Database
description: "Turn a codebase into a queryable code database. Scan once, query forever — no more grep/glob/Read. Supports C/C++/Go/Python/Java/Rust/ASM with invocation graphs, conditional paths, concurrency analysis, data flow, FFI tracing, and 19 cgdb semantic tables. 81 MCP tools (53 base + 28 design-report) + 222 CLI commands. Use /Code2Database when the question involves code structure, call chains, impact analysis, concurrency, or data flow."
...
python3 scripts/code2database_builder.py serve --graph code2db-out/  # MCP server (81 tools)
```
...
| `serve` | Start MCP server (81 tools) | All |
...
All 222 CLI commands remain accessible; the 24 above cover ~95% of agent workflows.
