...
Exposes **49 MCP tools** (30 `code2database_*` + 19 `cgdb_*`):

**30 `code2database_*` tools** (always available):
...
**18 `cgdb_*` tools** (require clang extraction backend — `--extraction-backend clang` or auto with libclang installed):
