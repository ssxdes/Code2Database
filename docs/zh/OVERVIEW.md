...
│   ├── mcp_server.py             ← MCP 服务器（stdio 传输，81 个工具 (53 base + 28 design-report)：34 code2database_* + 19 cgdb_*）
...
| 组件 | 用途 |
|------|------|
| **MCP（Model Context Protocol）stdio 传输** | `serve` 命令通过 JSON-RPC 暴露 81 个工具 (53 base + 28 design-report)（34 个 `code2database_*` + 19 个 `cgdb_*`），带 Content-Length 帧 |
...
`serve` 通过 stdio JSON-RPC 暴露 81 个工具 (53 base + 28 design-report)，带 Content-Length 帧：
