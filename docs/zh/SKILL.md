---
name: Code2Database
description: "将代码库转为可查询的代码数据库。扫描一次，永久查询——不再需要 grep/glob/Read。支持 C/C++/Go/Python/Java/Rust/ASM，调用图、条件路径、并发分析、数据流、FFI 追踪、19 个 cgdb 语义表。50 个 MCP 工具 + 184 个 CLI 命令。当代码问题涉及结构、调用链、影响面、并发或数据流时使用 /Code2Database。"
...
python3 scripts/code2database_builder.py serve --graph code2db-out/  # MCP 服务器（50 工具）
```
...
| `serve` | 启动 MCP 服务器（50 工具） | 全部 |
| `web-ui` | 交互式浏览器（cytoscape.js） | 全部 |
| `tx-begin` | 开始事务 | Ops |
| `daemon` | 后台自动同步 | Ops |
| `health` | 图谱新鲜度 + profile 健康 | — |

全部 184 个 CLI 命令仍可访问；上述 24 个覆盖 ~95% 的 agent 工作流。
...
50 工具：31 个 `code2database_*`（含新增 `code2database_kb_query` 跨 memory+knowledge 查询）+ 19 个 `cgdb_*`（clang 语义层）。
