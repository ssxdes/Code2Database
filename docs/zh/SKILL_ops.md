...
| `serve` | MCP 服务器模式（stdio，82 个工具 (54 base + 28 design-report)：35 code2database_* + 19 cgdb_*） |
...
- **MCP 服务器**：`serve` 暴露 82 个工具 (54 base + 28 design-report)（35 个 `code2database_*` + 19 个 `cgdb_*`）；无论子技能是否激活，全部可访问
