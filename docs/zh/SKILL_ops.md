...
| `serve` | MCP 服务器模式（stdio，48 个工具：30 code2database_* + 18 cgdb_*） |
...
- **MCP 服务器**：`serve` 暴露 48 个工具（30 个 `code2database_*` + 18 个 `cgdb_*`）；无论子技能是否激活，全部可访问
