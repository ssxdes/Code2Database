            ...
            if alias_nid and original_nid and alias_nid != original_nid:
                if not G.has_edge(alias_nid, original_nid):
                    G.add_edge(
                        ...
                        # build loop's default-concurrency logic only covers
                        # CALLBACK_ARG and EXTRACTED-confidence direct calls,
                        # not edges created here in Phase 8.
                        concurrency="alias",
                        evidence="asm alias: same symbol under different name",
                        source="asm_alias",
...
def _reclassify_external_domains(G, profile) -> int:
    ...
    from _builder.graph.graph_build import _is_external_domain
    reclassed = 0
    for nid, ndata in G.nodes(data=True):
        if ndata.get("is_empty", False):
            continue
