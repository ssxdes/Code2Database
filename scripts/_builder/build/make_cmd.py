    ...
    for name, cmd, fatal, note, requires, skip_reason in steps:
        ...
        if rc == 0 and name == "scan":
            ...
            if _errs:
                ...
                sys.exit(1)
            # Hand the auto-generated profile to the build step: the
            # scan ran with --auto-profile and wrote
            # <source>/.code2database_profile.json, but the build cmd
            # was assembled without it — so every profile-driven build
            # feature (external-prefix filtering, threading models,
            # endpoint classification) silently missed the run.
            if not getattr(args, "profile", ""):
                _auto_profile = os.path.join(
                    rep["source"], ".code2database_profile.json")
                if os.path.isfile(_auto_profile):
                    for _i, _step in enumerate(steps):
                        if _step[0] == "build" and "--profile" not in _step[1]:
                            steps[_i] = (_step[0],
                                         _step[1] + ["--profile", _auto_profile],
                                         _step[2], _step[3], _step[4],
                                         _step[5])
                            print("[make] auto-profile generated by scan "
                                  "-> forwarding to build: %s"
                                  % _auto_profile)
                            break
        if rc != 0:
