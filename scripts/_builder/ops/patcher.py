...
def patch_from_diff(graph_dir: str, diff_text: str, source_root: str = ""):
    ...
    if added_nodes > 0 or stale_nodes > 0 or removed_nodes > 0:
        ...
        try:
            from _builder.query.query_cache import invalidate_node, invalidate_all
            ...
            pass
        try:
            from _builder.ops.audit_log import log_audit, new_tx_id
            tx_id = new_tx_id()
            for nid in patched_nodes:
                log_audit(graph_dir,
...
def cmd_patch_from_diff(args):
    ...
    try:
        from _builder.ops.transactions import transaction
    except ImportError:
